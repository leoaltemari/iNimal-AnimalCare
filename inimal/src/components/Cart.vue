  ...
  <div>
    <div id="cart-content">
      ...
      <div v-if="!cart.length" class=error>
        <div class="message">
          <img src="../assets/img/icons/box_red_icon.png" alt="">
          <h1>Você não possui nenhum item no carrinho ainda</h1>
          <router-link to="/products">Visite a sessão de produtos</router-link>
        </div>
      </div>
      ...
      <div id="cart-table" v-else>
        ...
        <table>
          ...
          <tr v-for="item in cart" :key="item._id">
            ...
            <td>{{item.quantity}}</td>
            <td>R${{item.quantity * item.product.price}},00</td>
          </tr>
        </table>
  ...
  methods: {
    ...
    async finishOrder() {
      if(!this.checkCardNumber()) {
        ...
        this.orderErrors = "Por favor, insira o número do cartão";
        this.orderSuccess = false;
        return;
      } else if(!this.validateCardNumber()) {
        this.orderError = true;
        this.orderErrors = "Número do cartão é inválido";
        this.orderSuccess = false;
        return;
      } else {
